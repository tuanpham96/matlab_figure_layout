...
%             res = second_parse;
        ...
        function obj = apply_multiple_transforms(obj)
        ...
        function res_child = recursive_children_first_parse(parent, dimensions)
            res_child = struct();
            ...
            for i_field = 1:length(FigureLayout.possible_fields)
                ...
                if ~isnan(val)
                    if strcmp(field, 'transform')
                        val = FigureLayout.parse_transform_text(val, max_width, max_height);
                    end
                    res_child.(field) = val;
                end
            end
            if isfield(res_child, 'LABEL') || ...
                ...
                res_child = FigureLayout.normalize_dimensions(...
                    res_child, max_width, max_height);
            end
        ...
        function res = parse_transform_text(transform_text, max_width, max_height)
            ...
            res.E = res.E/max_width;
            res.F = res.F/max_height;
        end
